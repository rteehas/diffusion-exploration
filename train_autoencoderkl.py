...
@torch.no_grad()
def log_validation(vae, args, accelerator, weight_dtype, step, is_final_validation=False, validation_images=None):
    ...
    for i, validation_image in enumerate(validation_images):
        # validation_image = validation_image.convert("RGB") #Image.open(validation_image).convert("RGB")
...
def make_train_dataset(args, accelerator):
    ...
    #     sample_idxs = np.random.choice(list(range(dataset['test'].num_rows)), size=100, replace=False).tolist()
    #     sampled_rows = dataset['test'].select(sample_idxs)
    #     validation_images = sampled_rows['img']
    # else:
    #     data_files = {}
    ...
    sample_idxs = np.random.choice(list(range(dataset['test'].num_rows)), size=100, replace=False).tolist()
    sampled_rows = dataset['test'].select(sample_idxs)
    validation_images = sampled_rows['image']
    ...
    return train_dataset, validation_images
...
def main(args):
    ...
    if args.pretrained_model_name_or_path is None and args.model_config_name_or_path is None:
        ...
        vae_config["in_channels"] = 1
        vae_config["out_channels"] = 1
        vae_config["sample_size"] = args.resolution
        vae = AutoencoderKL.from_config(vae_config)
        vae.encoder.conv_in  = torch.nn.Conv2d(1, 128, kernel_size=3, stride=1, padding=1)
        vae.decoder.conv_out = torch.nn.Conv2d(128, 1, kernel_size=3, stride=1, padding=1)
    ...
    train_dataset, validation_images = make_train_dataset(args, accelerator)

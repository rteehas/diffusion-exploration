...
class ModifiedDDPOTrainer(PyTorchModelHubMixin):
    """
    ...
    def __init__(
        ...
        self.knockout_class = knockout_class
        ...
        self.reward_fn = reward_function(self.accelerator, self.sd_pipeline.sd_pipeline.image_processor)
        ...
        if hasattr(self.sd_pipeline, "use_lora") and self.sd_pipeline.use_lora:
            ...
            print("Trainable layers:", self.trainable_layers)
        else:
    ...
    def compute_rewards(self, prompt_image_pairs, step, is_async=False, log_rewards=True):
        if not is_async:
            rewards = []
            for images, prompts, prompt_metadata in prompt_image_pairs:
                reward, reward_metadata = self.reward_fn(images, prompts, prompt_metadata, step, log=log_rewards)
                rewards.append(
                    (
    ...
    def step(self, epoch: int, global_step: int):
        ...
        rewards, rewards_metadata = self.compute_rewards(
            prompt_image_data, step=global_step, is_async=self.config.async_reward_computation, log_rewards=True
        )
    ...
    def generate_eval_samples(self, eval_prompts: list[str], batch_size: int | None = None):
        """
    ...
    def train(self, epochs: Optional[int] = None):
        ...
        for epoch in range(self.first_epoch, epochs):
            ...
            if epoch % eval_every == 0 and self.eval_prompts:
                    if self.knockout_class is not None:
                        knockout_prompts = [p for p in self.eval_prompts if self.knockout_class in p]
                        prompts = [p for p in self.eval_prompts if self.knockout_class not in p]

                        self.evaluate(knockout_prompts, step=global_step, eval_name="knockout_validation")
                        self.evaluate(prompts, step=global_step, eval_name="trained_validation")
                    else:
                        self.evaluate(self.eval_prompts, step=global_step, eval_name="validation_all_prompts")
    @torch.no_grad
    def evaluate(
        self,
